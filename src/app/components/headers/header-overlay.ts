...
@Component({
    ...
    <header class="bg-white shadow-sm relative z-20">
      <!-- Cover with Overlay -->
      <div class="h-56 md:h-72 w-full bg-gradient-to-r from-gray-800 to-gray-900 overflow-hidden relative">
         @if (tenant?.coverUrl) {
           <img [src]="tenant?.coverUrl" alt="Cover" class="w-full h-full object-cover">
           <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-black/20"></div>
         } @else {
           <div class="absolute inset-0 bg-gradient-to-r from-terra to-terra-800"></div>
         }
         
         <!-- Content Overlayed on Cover -->
         <div class="absolute inset-0 flex items-end">
             <div class="container mx-auto px-4 pb-6 flex items-end gap-6">
                 <!-- Logo -->
                 <div class="h-24 w-24 md:h-32 md:w-32 rounded-full border-4 border-white shadow-xl overflow-hidden bg-white flex-shrink-0">
                     @if (tenant?.logoUrl) {
                         <img [src]="tenant?.logoUrl" [alt]="tenant?.name" class="w-full h-full object-cover">
                     } @else {
                         <div class="w-full h-full flex items-center justify-center bg-terra text-white text-3xl font-bold">
                 ...
                 <div class="flex-1 mb-2">
                     <h1 class="text-3xl md:text-4xl font-bold text-white leading-tight drop-shadow-lg">{{ tenant?.name || slug }}</h1>
                     @if (tenant?.bio) {
                         <p class="text-white/90 text-base md:text-lg mt-2 drop-shadow-md line-clamp-2">{{ tenant?.bio }}</p>
                     }
                     
                     <!-- Social Icons -->
                     <div class="flex gap-3 mt-3">
                         @if (tenant?.socialFacebookEnabled && tenant?.socialFacebook) { 
                             <a [href]="tenant!.socialFacebook" target="_blank" class="text-white/70 hover:text-white transition-colors">
                                 <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                             </a>
                         }
                         @if (tenant?.socialInstagramEnabled && tenant?.socialInstagram) { 
                             <a [href]="tenant!.socialInstagram" target="_blank" class="text-white/70 hover:text-white transition-colors">
                         ...
                         @if (tenant?.socialTiktokEnabled && tenant?.socialTiktok) { 
                             <a [href]="tenant!.socialTiktok" target="_blank" class="text-white/70 hover:text-white transition-colors">
                         ...
                         @if (tenant?.socialYoutubeEnabled && tenant?.socialYoutube) { 
                            <a [href]="tenant!.socialYoutube" target="_blank" class="text-white/70 hover:text-white transition-colors">
                            ...
                            @if (tenant?.socialWhatsappEnabled && tenant?.socialWhatsapp) { 
                                <a [href]="'https://wa.me/' + tenant!.socialWhatsapp" target="_blank" class="text-white/70 hover:text-white transition-colors">
                 ...
                 <!-- Search + Cart - White Styled -->
                 <div class="hidden md:flex gap-3 items-center mb-2">
                     <!-- Search -->
                     <div class="relative">
                         <input type="text" 
                                placeholder="Buscar..." 
                                class="w-48 pl-10 pr-4 py-2 bg-white/20 backdrop-blur border border-white/30 rounded-full text-sm text-white placeholder:text-white/70 outline-none focus:bg-white/30"
                                [value]="searchQuery"
                                (keyup.enter)="onSearch($event)"
                                (input)="onSearchInput($event)">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="absolute left-3 top-2.5 text-white/70"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                          @if (searchQuery) {
                            <button (click)="onClearSearch()" class="absolute right-3 top-2.5 text-white/70 hover:text-white">
                     ...
                     <button (click)="cart.open()" class="relative bg-white/20 backdrop-blur text-white rounded-full p-2.5 border border-white/30 hover:bg-white/30 transition-colors">
                        ...
                        @if (cart.count() > 0) {
                            <span class="absolute -top-1 -right-1 bg-terra text-white text-xs font-bold h-5 w-5 rounded-full flex items-center justify-center">{{ cart.count() }}</span>
                        }
                     </button>
                 </div>
             </div>
      ...
      <!-- Mobile Search + Cart -->
      <div class="md:hidden container mx-auto px-4 py-3 flex gap-3">
